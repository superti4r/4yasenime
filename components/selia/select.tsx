...
export function SelectValue({
  ...
  ...props
}: React.ComponentProps<typeof BaseSelect.Value> & { placeholder?: string }) {
  return (
    <BaseSelect.Value
...
export function SelectPopup({
  ...
  return (
    <BaseSelect.Portal>
      <BaseSelect.Backdrop className="fixed inset-0 z-[2147483646] bg-black/0" />
      ...
      >
        <BaseSelect.ScrollUpArrow className="top-1 left-1 right-1 z-[2147483647] absolute rounded h-5 text-xs text-popover-foreground flex items-center justify-around bg-popover-accent" />
        <BaseSelect.Popup
          data-slot="select-popup"
          {...props}
          className={cn(
            "relative z-[2147483647] group origin-[var(--transform-origin)] bg-popover ring ring-popover-border rounded shadow-popover",
            "p-1 outline-none w-[var(--anchor-width)] max-w-[var(--available-width)] overflow-hidden",
            className
          )}
        >
          <BaseSelect.Arrow />
        ...
        <BaseSelect.ScrollDownArrow className="bottom-1 left-1 right-1 z-[2147483647] absolute rounded h-5 text-xs text-popover-foreground flex items-center justify-around bg-popover-accent" />
      </BaseSelect.Positioner>
    </BaseSelect.Portal>
  );
}
...
export function SelectList({
  ...
  return (
    <BaseSelect.List
      ...
      className={cn(
        "space-y-0.5 max-h-[var(--available-height)] overflow-y-auto relative",
        className
      )}
      {...props}
    />
